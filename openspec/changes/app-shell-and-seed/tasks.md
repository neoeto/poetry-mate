...
## 3. 字体与排版基线
  ...
  - 冒烟测试: 文楷族渲染含繁体/生僻样例文本不崩溃
  - 思源宋体不打包(Flutter 无声明式回退链,生僻字由系统 CJK 兜底); 再生成流程见 etl/docs/font-charset.md
  - 剩余缺口均为 CJK 扩展 B/F 罕见异体(全量 GB 字体本就不含) → 系统字体回退,v1 接受
  - 首版踩坑: 只用原文字符做子集,漏掉繁转简新生字(癘→疠)致种子缺 20 字 → v2 字符集修复至仅剩 2 字
  - 语料子集化: 25.8MB → 8.9MB(15,069 字形), 字符集=出货简体文本 ∪ 原文 ∪ 标点(15,740)
- [x] 3.1 打包霞鹜文楷 GB 版到 assets，配置字体栈（文楷 → 思源宋体 → 系统），验证生僻字回退无豆腐块
- [x] 3.2 建立双轨 TextTheme：内容族文楷 / 界面族系统黑体，全应用生效
  - AppTheme.contentTextStyle(context): 文楷+24sp默认+行高2.0, 调用方仅可覆写尺寸/颜色
  - AppTheme.uiTextStyle(context): 系统黑体显式声明; 双轨契约由样式对象测试钉死(4用例)
- [x] 3.3 简读页基线版式：一句一行左对齐、≥22sp、行距≥1.9、标题居中、落款偏右弱化
  - ReaderPage(poem): 标题居中文楷/落款右对齐弱墨色/0.5细线分隔/正文一句一行
  - 4 个版式测试: 绝句四行/词长短句/样式契约(≥22sp,≥1.9,文楷)/标题居中+落款偏右

## 4. 导航壳与页面

- [x] 4.1 底部四 Tab 导航壳，Tab 状态保持
  - 1.3 已完成 StatefulShellRoute.indexedStack; 本轮接入真实分支页装配 _branchPage
  - 空库语义测试: 分类空态/深链未找到 均为友好文案而非崩溃
- [x] 4.2 分类页：朝代 × 类型两级浏览种子集列表，点击进简读页
  - 过滤chips(全部/唐诗/宋诗/宋词) + 列表(标题回退词牌·作者·热度) + 深链进阅读
  - main() 异步引导: 文件库 + 种子装载(失败兜底空库不阻断启动)
  - 集成测试: 分类列出→宋词过滤→点进阅读正文呈现; 空库空态; 共2用例
...
- [x] 5.1 断网端到端验收：飞行模式下全新安装 → 冷启动 → 浏览 → 打开《静夜思》简读页，全程零网络请求、≤10 秒
  - iOS 模拟器全流程通过(2026-08): 卸载→安装→启动→300条入库, 实际耗时约2秒(远低于10s上限)
  - 零网络证明: lib/ 静态审计零网络调用(无 HttpClient/http/fetch) + 种子资产本地内置
  - ⚠️ 物理设备飞行模式复测并入 5.3 一并执行
- [x] 5.2 二次启动路径验证：无重复写入、无解压发生（日志/断点确认）
  - iOS 模拟器实测: 冷重启后行数 300→300, DB 文件 mtime 未变(零写入零解压)
  - 统一日志捕获 [seed] SeedLoadAction.skipped —— 跳过路径日志证据完整
